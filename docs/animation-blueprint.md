...
V2에서 완성한 것은 두 까치의 **통합 시뮬레이션**이다. 아직 최종 17개 AGIF, WFF 연결, 에뮬레이터·실기기 구현은 완료되지 않았다.
...
- 구도상 필요할 때 실제 방향 전환 최대 1회
- 즉시 좌우 미러 금지. 전환 AGIF가 미러 변경을 담당
...
- 오른쪽 퇴장 중 정확히 두 번 날갯짓하고 완전히 화면 밖으로 이동
...
- V2의 렌더 전용 실루엣 마스크는 모션 검증용 초안일 뿐 최종 마스터가 아니다.
- 최종 AGIF 전에 깨끗한 작은 까치 마스터와 발 기준점을 확정한다.
...
현재 자동 테스트는 결정성·교대·한 마리·상태 연결·실제 착지 빈도·양쪽 바늘 사용·쪼기·방향 전환·날갯짓·AOD·자정 연속성을 검사한다.

정적 이미지나 미리보기 GIF만 만든 경우 `애니메이션 구현 완료`라고 보고하지 않는다.

## 17. V2 승인 뒤 제작 순서

1. V2 GIF의 구도·성격·빈도 승인
2. 깨끗한 작은 까치 기반 마스터와 발 기준점 확정
3. 17개 AGIF 원본 프레임·메타데이터 제작
4. 정적 자세 6개와 전경 가림막 3개 확정
5. 공통 무상태 결정식을 WFF 표현식으로 이식
