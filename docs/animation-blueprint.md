...
V4에서 두 까치의 정적 자세 6종, 전경 가림막 3종, 16개 AGIF와 원본 프레임·메타데이터를 완성했다. 배경에 구워져 있던 중앙 표시 잔상·구름선과 배터리를 제거하고 실시간 WFF 텍스트로 교체했다. 날짜 뒤 `(188, 272)~(270, 294)` 한지 덮개는 배경 바로 위·시침과 분침 아래에 두어 잔상은 지우되 회전 바늘은 가리지 않는다. 94% 호랑이 뒤에 남은 원래 100% 뒷다리 외곽선은 승인된 94% 소스로 교체했다. 배터리 매화와 소나무·호랑이 전경 마스크 및 호랑이 머리를 시침·분침보다 먼저 렌더해 어느 각도에서도 바늘이 사라지지 않게 한다. 호랑이 위 까치는 머리보다 위에서 그리며 발 기준점은 큰 까치 `(335, 233)`, 작은 까치 `(340, 241)`다. 작은 까치는 머리 끝과 6px 겹쳐 손목 크기에서도 접촉이 분명하다. 정적 호랑이 머리와 반응 AGIF는 한 `Condition`에서 배타적으로 전환해 움직임이 정적 머리에 상쇄되지 않게 하며, 반응 분의 AOD에는 기본 알파 0에서 ambient 알파만 복원되는 정적 머리·눈동자 백업을 사용한다. 작은 고정 비행 자세는 정적 자세와 높이를 맞춘 `70×54`다. 상단 한지 여백의 솔잎 조각 `(282, 158)~(306, 181)`도 인접 종이 질감으로 제거했다. 중앙 표시는 모든 장식 레이어보다 위에 두고 세로 20px를 내린 채 가로 중심을 다이얼 정중앙 `x=225`에 맞춘다. 43분 무상태 시간표·시침·분침 캐리어·AOD까지 `watchface.xml`에 연결했고 50개 자동 테스트가 통과한다. APK 조립과 Wear OS 에뮬레이터·실기기 렌더 검증은 Android SDK가 있는 환경에서 수행한다.
...
```text
대화형 모드 11.62MiB 추정 / 공식 100MiB 미만
