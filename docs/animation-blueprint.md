...
V4에서 두 까치의 정적 자세 6종, 전경 가림막 3종, 16개 AGIF와 원본 프레임·메타데이터를 완성했다. 배경에 구워져 있던 중앙 표시 잔상·구름선과 배터리를 제거하고 실시간 WFF 텍스트로 교체했으며, 날짜 뒤 옅은 구름 잔상은 `(198, 250)~(252, 300)` 전체를 종이 질감으로 복원했다. 상단 한지 여백에 고립된 솔잎 조각 `(282, 158)~(306, 181)`도 인접 종이 질감으로 제거했다. 중앙 표시는 모든 장식 레이어보다 위에 두고 세로 20px를 내린 채 가로 중심을 다이얼 정중앙 `x=225`에 맞춘다. 43분 무상태 시간표·배터리 매화·시침·분침 캐리어·AOD까지 `watchface.xml`에 연결했고 49개 자동 테스트가 통과한다. APK 조립과 Wear OS 에뮬레이터·실기기 렌더 검증은 Android SDK가 있는 환경에서 수행한다.
