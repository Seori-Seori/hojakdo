...
> 문서 상태: V4.3 상단 판독성 개선 후보 구현 완료 기준서
>
...
V4에서 두 까치의 정적 자세 6종, 전경 가림막 3종, 16개 AGIF와 원본 프레임·메타데이터를 완성했다. V4.1은 시침을 92px, 분침을 126px로 조정하고 시침을 1.24배 굵기·0.78배 밝기로 만들어 손목 크기에서도 즉시 구분되게 한다. 시침 큰/작은 까치 착석점은 `(293, 152)`·`(290, 157)`, 분침은 `(155, 114)`·`(168, 118)`이며 비행·착지 끝점도 같은 변환을 사용한다. 상단 판독성 개선안은 하이브리드 워치페이스의 상단 1/3 배치를 기준으로 실시간 `HH:mm`을 `x=225`, `y=85`, 52px로 키우고 `MM.DD  요일`을 `y=132`, 16px 한 줄로 합친다. 실시간 표시는 모든 바늘·까치·장식보다 위에 두며, 하단 배터리 아이콘과 고정 폭 숫자 영역은 하나의 62px 묶음으로 다이얼 중앙에 정렬한다. 배경에 구워져 있던 중앙 표시 잔상·구름선과 배터리를 제거하고 실시간 WFF 텍스트로 교체했다. 날짜 뒤 `(188, 272)~(270, 294)` 한지 덮개는 배경 바로 위·시침과 분침 아래에 두어 잔상은 지우되 회전 바늘은 가리지 않는다. 94% 호랑이 뒤에 남은 원래 100% 뒷다리 외곽선은 승인된 94% 소스로 교체했다. 배터리 매화와 소나무·호랑이 전경 마스크 및 호랑이 머리를 시침·분침보다 먼저 렌더해 어느 각도에서도 바늘이 사라지지 않게 하고, 매화 걷기 까치는 가지와 꽃 위에 그려 만개 상태에서도 숨지 않는다. 호랑이 위 까치는 머리 레이어보다 위에서 그리며 발 기준점은 큰 까치 `(335, 233)`, 작은 까치 `(340, 241)`다. 작은 까치는 머리 끝과 6px 겹쳐 손목 크기에서도 접촉이 분명하다. 정적 호랑이 머리와 반응 AGIF는 한 `Condition`에서 배타적으로 전환해 움직임이 정적 머리에 상쇄되지 않게 하며, 반응 분의 AOD에는 기본 알파 0에서 ambient 알파만 복원되는 정적 머리·눈동자 백업을 사용한다. 작은 고정 비행 자세는 정적 자세와 높이를 맞춘 `70×54`다. 상단 한지 여백의 솔잎 조각 `(282, 158)~(306, 181)`도 인접 종이 질감으로 제거했다. 43분 무상태 시간표·시침·분침 캐리어·AOD까지 `watchface.xml`에 연결했고 51개 자동 테스트가 통과한다. APK 조립과 Wear OS 에뮬레이터·실기기 렌더 검증은 Android SDK가 있는 환경에서 수행한다.
...
현재 전체 자동 테스트는 결정성·교대·한 마리·상태 연결·실제 착지 빈도·양쪽 바늘 사용·쪼기·실제 이동 방향·호랑이 턱 복원·V2.2 구도·배터리 경계·숫자 표시 보존·매화 자산 메모리·AOD·자정 연속성에 더해, V3.1 승인 원본 해시·자산 재현성·고정 비행 스프라이트·날갯짓 0회·화면 밖 완전 퇴장·V4.3 상단 두 줄 표시를 검사한다.
