...
V2.3에서 완성한 것은 두 까치의 **통합 시뮬레이션, V2.2 구도 보정, 매화 배터리 5단계 정적 자산**이다. 아직 최종 17개 AGIF, WFF 연결, 에뮬레이터·실기기 구현은 완료되지 않았다.
...
현재 총 36개 자동 테스트는 결정성·교대·한 마리·상태 연결·실제 착지 빈도·양쪽 바늘 사용·쪼기·실제 이동 방향·날개 접합·날개 비율·호랑이 턱 복원·V2.2 구도·배터리 경계·숫자 표시 보존·매화 자산 메모리·AOD·자정 연속성을 검사한다.

정적 이미지나 미리보기 GIF만 만든 경우 `애니메이션 구현 완료`라고 보고하지 않는다.

## 17. V2.3 승인 뒤 제작 순서

1. V2.3 구도·성격·빈도·매화 배터리 승인
2. 17개 AGIF 원본 프레임·메타데이터 제작
3. 작은 까치 퇴장 날개 독립 자산 마무리
4. 정적 자세 6개와 전경 가림막 3개 확정
5. 공통 무상태 결정식과 매화 배터리 조건을 WFF 표현식으로 이식
6. Transform과 렌더링 슬롯 연결
7. AOD와 메모리 예산 검증
8. Wear OS 에뮬레이터 검증
9. 가능하면 실제 갤럭시 워치 검증
