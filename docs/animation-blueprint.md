...
> 문서 상태: V2.3 매화 배터리 5단계 완료 + V3 작은 까치 고정 비행 승인 기준서
>
...
V2.3에서 완성한 것은 두 까치의 **통합 시뮬레이션, V2.2 구도 보정, 매화 배터리 5단계 정적 자산**이다. 2026-07-15에는 V3 작은 까치의 오른쪽 고정 비행 원본을 승인했다. V3 제작 목표는 16개 AGIF와 작은 까치 고정 비행 PNG 1장이다. 아직 투명 런타임 마스터 파생, WFF 연결, 에뮬레이터·실기기 구현은 완료되지 않았다.
...
- 몸통의 접힌 날개가 중복되어 보이지 않는 최종 승인본을 기준으로 투명 런타임 마스터 파생
...
현재 총 36개 자동 테스트는 결정성·교대·한 마리·상태 연결·실제 착지 빈도·양쪽 바늘 사용·쪼기·실제 이동 방향·날개 접합·날개 비율·호랑이 턱 복원·V2.2 구도·배터리 경계·숫자 표시 보존·매화 자산 메모리·AOD·자정 연속성을 검사한다. 이 테스트의 작은 까치 2회 날갯짓 항목은 V2.3 회귀 기준이며, V3 통합 때 고정 비행 스프라이트·날갯짓 0회·가운데 디지털 시각·요일 보존 검사로 교체한다.

정적 이미지나 미리보기 GIF만 만든 경우 `애니메이션 구현 완료`라고 보고하지 않는다.

## 17. V3 제작 순서

1. 승인된 작은 까치 비행 원본에서 한지 배경을 제거하고 투명 정렬 런타임 마스터 파생
2. V2.3 프로토타입의 2회 날갯짓 퇴장을 고정 스프라이트 교체 + 오른쪽 전체 이동으로 변경
3. V3 통합 미리보기에서 화면 가운데 디지털 시각·요일, 배터리 아이콘·숫자 퍼센트, 매화 5단계가 그대로인지 확인
4. 16개 AGIF 원본 프레임·메타데이터 제작
5. 정적 자세 7개와 전경 가림막 3개 확정
