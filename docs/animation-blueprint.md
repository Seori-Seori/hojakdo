...
> 문서 상태: V4.3.2 레이어 정리·호랑이 visual slot 준비 구현 기준서
>
...
V4에서 두 까치의 정적 자세 6종, 16개 AGIF와 원본 프레임·메타데이터를 완성했다. V4.3.2에서는 최종 배경과 중복되던 매화·소나무 전경 마스크와 별도 한지 덮개를 런타임에서 제거하고, 호랑이 몸 호환 마스크 1종만 다음 호랑이 재구성 전까지 유지한다. V4.1은 시침을 92px, 분침을 126px로 조정하고 시침을 1.24배 굵기·0.78배 밝기로 만들어 손목 크기에서도 즉시 구분되게 한다. 시침 큰/작은 까치 착석점은 `(293, 152)`·`(290, 157)`, 분침은 `(155, 114)`·`(168, 118)`이며 비행·착지 끝점도 같은 변환을 사용한다. 상단 판독성 개선안은 하이브리드 워치페이스의 상단 1/3 배치를 기준으로 실시간 `HH:mm`을 `x=225`, `y=85`, 52px로 키우고 `MM.DD  요일`을 `y=132`, 16px 한 줄로 합친다. V4.3.1은 `ON_VISIBLE` 재생기를 제거하고 모든 동작 프레임을 `[SECOND_MILLISECOND]`로 직접 선택한다. 따라서 화면 재점등이나 워치페이스 이탈·복귀 때 같은 절대 시각에는 같은 프레임이 복원되며, 각 동작의 마지막 프레임은 그 분 끝까지 유지된다. 폭이 거의 0이 되던 큰·작은 까치 방향 전환 중간 프레임은 런타임에서 건너뛴다. 실시간 표시는 모든 바늘·까치·장식보다 위에 두며, 하단 배터리 아이콘과 고정 폭 숫자 영역은 하나의 62px 묶음으로 다이얼 중앙에 정렬한다. 배경에 구워져 있던 중앙 표시 잔상·구름선과 배터리를 제거하고 실시간 WFF 텍스트로 교체했다. 날짜 뒤 `(188, 272)~(270, 294)` 영역은 최종 배경 자체에서 한지 질감으로 복원하며 별도 한지 덮개 레이어는 사용하지 않는다. 94% 호랑이 뒤에 남은 원래 100% 뒷다리 외곽선은 승인된 94% 소스로 교체했다. 배터리 매화와 매화 걷기 까치는 최종 배경 위에 직접 렌더하고, 호랑이 몸 호환 마스크·머리·눈동자·기존 반응을 하나의 `tiger_visual_slot`으로 묶어 시침·분침보다 먼저 렌더한다. 호랑이 위 까치는 머리 레이어보다 위에서 그리며 발 기준점은 큰 까치 `(335, 233)`, 작은 까치 `(340, 241)`다. 작은 까치는 머리 끝과 6px 겹쳐 손목 크기에서도 접촉이 분명하다. 정적 호랑이 머리와 반응 프레임은 한 `Condition`에서 배타적으로 전환해 움직임이 정적 머리에 상쇄되지 않게 하며, 반응 분의 AOD에는 기본 알파 0에서 ambient 알파만 복원되는 정적 머리·눈동자 백업을 사용한다. 작은 고정 비행 자세는 정적 자세와 높이를 맞춘 `70×54`다. 상단 한지 여백의 솔잎 조각 `(282, 158)~(306, 181)`도 인접 종이 질감으로 제거했다. 43분 무상태 시간표·시침·분침 캐리어·AOD까지 `watchface.xml`에 연결한다. APK 조립과 Wear OS 에뮬레이터·실기기 렌더 검증은 Android SDK가 있는 환경에서 수행한다.
...
WFF v1의 `AnimationController`는 현재 절대 시각의 임의 AGIF 프레임으로 탐색할 수 없다. 런타임에서는 각 AGIF를 구성한 PNG 프레임을 개별 리소스로 패키징하고 `[SECOND_MILLISECOND]` 조건으로 직접 선택한다. 마지막 프레임 조건에는 상한을 두지 않아 해당 분이 끝날 때까지 도착 자세를 유지한다. 이 구조에서는 화면 재점등과 워치페이스 재진입이 재생 시작점에 영향을 주지 않는다.
...
- 상단 중앙에 큰 디지털 시각을 두고 날짜·요일은 바로 아래 한 줄로 유지한다.
...
- 오른쪽 퇴장은 승인된 `magpie_small_flight_right_v3_approved.png`에서 파생한 70×54 한 장짜리 고정 자세를 사용하며 정적 작은 까치와 높이를 맞춘다
...
- `plum_foreground_mask.png`와 `pine_foreground_mask.png`는 V4.3.2에서 제거했다. 두 영역은 최종 배경에 이미 포함되어 있어 같은 픽셀을 다시 올리는 중복 레이어였다.
- `tiger_body_foreground_mask.png`는 호랑이 몸이 아직 배경에 구워진 현재 구조를 위한 임시 호환 레이어다.
- 다음 호랑이 작업에서는 이 호환 마스크와 기존 머리·눈동자 조각을 `tiger_visual_slot`의 전체 호랑이 프레임으로 함께 교체한다. 호랑이 프레임 자체는 사용자와 한 장씩 검수한 뒤 만든다.
...
`tiger_visual_slot`은 현재 호랑이 몸 호환 마스크 + 정적 머리/눈동자 + 기존 머리 반응을 감싼 경계다. 새 앞발·그루밍·시선 추적 프레임은 아직 만들지 않았으며, 다음 단계에서 이 슬롯 안의 호랑이 전체 이미지를 한 장씩 검수해 교체한다.
...
```text
대화형 모드 11.21MiB 추정 / 공식 100MiB 미만
정적 자산 5.42MiB 추정 / 공식 10MiB 미만
```
...
상단 두 줄 디지털 시각·날짜·요일의 위치·형식·가시성 유지
...
현재 전체 자동 테스트는 결정성·교대·한 마리·상태 연결·실제 착지 빈도·양쪽 바늘 사용·쪼기·실제 이동 방향·호랑이 턱 복원·V2.2 구도·배터리 경계·숫자 표시 보존·매화 자산 메모리·AOD·자정 연속성에 더해, V3.1 승인 원본 해시·자산 재현성·고정 비행 스프라이트·날갯짓 0회·화면 밖 완전 퇴장·V4.3 상단 두 줄 표시·V4.3.1의 16개 동작 전 밀리초 가시성 재진입 복원을 검사한다·V4.3.2 중복 레이어 제거·단일 `tiger_visual_slot`·런타임 비참조 자산 제외까지 검사한다.
...
3. [완료] 통합 정적 화면에서 상단 디지털 시각·날짜·요일 레이아웃, 배터리 아이콘·숫자 퍼센트, 매화 5단계 보존 확인
...
사이클 43분, 첫 2분 숨김, 엄격 교대, 하루 평균 약 6회, 작은 까치의 고정 비행 스프라이트·날갯짓 0회·3회 중 2회 귀 쪼기, 큰 까치의 접은 날개 점프, 상단 두 줄 디지털 표시는 승인된 핵심 규칙으로 유지한다.
