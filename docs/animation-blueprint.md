...
```text
대화형 모드 11.61MiB 추정 / 공식 100MiB 미만
AOD 정적 자산 5.82MiB 추정 / 공식 10MiB 미만
```
...
현재 전체 자동 테스트는 결정성·교대·한 마리·상태 연결·실제 착지 빈도·양쪽 바늘 사용·쪼기·실제 이동 방향·호랑이 턱 복원·V2.2 구도·배터리 경계·숫자 표시 보존·매화 자산 메모리·AOD·자정 연속성에 더해, V3.1 승인 원본 해시·자산 재현성·고정 비행 스프라이트·날갯짓 0회·화면 밖 완전 퇴장·V4.3 상단 두 줄 표시·V4.3.1의 16개 동작 전 밀리초 가시성 재진입 복원을 검사한다.
