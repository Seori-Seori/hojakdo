# 호작도 V4.3.1 가시성 복원 수정

승인된 V3.1 구도와 작은 까치 고정 비행을 Watch Face Format v1에 연결하고, V4.1 바늘 판독성·V4.3 상단 실시간 표시·V4.3.1 화면 재진입 복원을 통합한 제작 파이프라인이다.
...
- 매화: 배터리 5단계 정적 교체
- 상단 표시: `HH:mm`을 `x=225`, `y=85`, 52px로 배치
- 배터리 표시: 아이콘과 고정 폭 숫자 영역을 62px 묶음으로 중앙 정렬
- 상단 보조줄: `MM.DD  요일`을 `x=225`, `y=132`, 16px 한 줄로 배치
- 실시간 표시 레이어: 바늘·까치·전경보다 위에 두어 항상 읽히게 유지
...
- 16개 원본 AGIF의 PNG 프레임을 `[SECOND_MILLISECOND]` 구간으로 선택해 화면 재점등·워치페이스 재진입에도 현재 프레임을 복원
- 모든 짧은 동작은 마지막 프레임을 해당 분 끝까지 유지해 착지·걷기·행동 직후 소실 방지
- 방향 전환 2종은 폭이 거의 0이 되는 중간 프레임을 건너뛰고 첫 자세→마지막 자세를 피영극식으로 즉시 전환
- AOD에서는 동작 프레임과 까치를 숨기고 배경·바늘·호랑이·현재 매화 단계·실시간 표시만 감광 유지
...
자동 검증은 16개 원본 AGIF와 런타임 PNG 프레임·6개 정적 자세·3개 가림막·5개 매화 단계·WFF 자원 참조·표현식 괄호·무상태 시간표·AOD 가시성·V4.1 바늘 길이비와 네 착석점·V4.3 상단 두 줄 표시·바늘 레이어·작은 까치 접촉 픽셀·호랑이 배타 반응·메모리 예산을 확인한다. 특히 16개 동작의 60,000개 밀리초 지점을 전수 검사해 화면을 껐다 켜거나 다른 워치페이스에서 돌아와도 같은 프레임을 선택하는지 검증한다.
