# 호작도 V4.2 상단 판독성 개선 후보

승인된 V3.1 구도와 작은 까치 고정 비행을 Watch Face Format v1에 연결하고, V4.1 바늘 판독성과 V4.2 상단 실시간 표시를 통합한 제작 파이프라인이다.
...
- 매화: 배터리 5단계 정적 교체
- 상단 표시: `HH:mm`을 `x=225`, `y=38`, 36px로 배치
- 상단 보조줄: `MM.DD  요일`을 `x=225`, `y=79`, 13px 한 줄로 배치
...
자동 검증은 16개 AGIF·6개 정적 자세·3개 가림막·5개 매화 단계·WFF 자원 참조·표현식 괄호·무상태 시간표·AOD 가시성·V4.1 바늘 길이비와 네 착석점·V4.2 상단 두 줄 표시·바늘 레이어·작은 까치 접촉 픽셀·호랑이 배타 반응·메모리 예산을 확인한다. 현재 저장소 전체 51개 테스트가 통과한다.
